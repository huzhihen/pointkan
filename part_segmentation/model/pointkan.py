...
class Model(nn.Module):
    def __init__(self, points=2048, class_num=50, embed_dim=64, res_expansion=1.0,
                 activation="relu", bias=False, use_xyz=True, normalize="center",
                 dim_expansion=[2, 2], kan_pre_blocks=[1, 1], kan_sub_blocks=[1, 1],
                 k_neighbors=[32, 32], reducers=[4, 4], de_dims=[256, 128], de_blocks=[1, 1],
        ...
        # KAN Encoder Network
        self.embedding = MLPExtraction(6, embed_dim, bias=bias, activation=activation)
        ...
        self.cls_map = nn.Sequential(
            Conv1dBNReLU(16, cls_dim, bias=bias, activation=activation),
            Conv1dBNReLU(cls_dim, cls_dim, bias=bias, activation=activation)
        )
    ...
    def forward(self, x, norm_plt, cls_label):
        ...
        x = torch.cat([x, norm_plt], dim=1)  # x: [B, 6, N]
        ...
        x = self.classifier(x)  # x: [B, 128 + 64 + 64, 2048] -> x: [B, 50, 2048]
        x = F.log_softmax(x, dim=1)  # x: [B, 50, 2048]
        x = x.permute(0, 2, 1)  # x: [B, 2048, 50]
        return x


def pointKAN(num_classes=50, **kwargs) -> Model:
